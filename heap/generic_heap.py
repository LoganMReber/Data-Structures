...
class Heap:
    ...
    def _bubble_up(self, index):
        ...
        if self.comparator(self.storage[index], self.storage[parent]):
            ...
            self._bubble_up(math.ceil((index/2)-1))
    ...
    def _sift_down(self, index):
        ...
        if len(self.storage) < right:
            ...
            if self.comparator(self.storage[right], self.storage[index]) and self.comparator(self.storage[right], self.storage[left]):
                self.storage[index], self.storage[right] = self.storage[right], self.storage[index]
                self._sift_down(right)
            elif self.comparator(self.storage[left], self.storage[index]):
                self.storage[index], self.storage[left] = self.storage[left], self.storage[index]
                self._sift_down(left)
